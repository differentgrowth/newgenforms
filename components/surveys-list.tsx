      ...
      { surveys.map( survey => (
        <Card key={ survey.id }>
          ...
          { !!survey.from && !!survey.to
            ...
            : null }
          <CardFooter className="grid grid-cols-2 sm:grid-cols-5 space-x-1.5">
            { survey.status === SURVEY_STATUS.READY || survey.status === SURVEY_STATUS.PUBLISHED
              ? (
                <CopySurveyUrlButton
                  surveyId={ survey.id }
                  className="col-span-2 sm:col-span-5 mb-1.5"
                />
              )
              : null }
            { survey.status === SURVEY_STATUS.EMPTY || survey.status === SURVEY_STATUS.PENDING
              ? (
                ...
                >
                  Edit
                  <Pencil1Icon className="ml-1.5 w-4 h-4" />
                </Link>
              )
              : null }
            ...
            { survey.status ===
              ...
              ? (
                ...
                >
                  View
                  <EyeOpenIcon className="ml-1.5 w-4 h-4" />
                </Link>
              )
              : null }
            <DeleteDialog
