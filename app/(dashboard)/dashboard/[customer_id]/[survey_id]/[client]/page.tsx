    ...
    <main className="container mx-auto py-10">
      ...
      <div className="mt-6 border-t">
        <dl className="divide-y">
          { data.map( item => (
            <div
              className="px-4 py-6 sm:grid sm:grid-cols-3 lg:grid-cols-5 sm:gap-4 sm:px-0"
              key={ item.id }
            >
              ...
              <dd className="mt-1 text-sm leading-6 sm:col-span-2 lg:col-span-4 sm:mt-0">
                <div className="flex flex-col gap-y-1.5">
                  <DateFormatter
                    date={ item.created_at }
                    timezone={ item.Question.Survey.timezone }
                    className="grow-0 mr-auto"
                  />
